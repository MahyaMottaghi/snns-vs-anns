...
class SpikingCNN(nn.Module):
    ...
    Architecture:
        ...
        Linear(800, 10)   ← NO LIF here (output is membrane potential)

    The output layer produces raw continuous values (like membrane potential),
    NOT binary spikes. This is standard practice in SpikingJelly — it gives
    CrossEntropyLoss smooth gradients to work with. Method 1 (SNNTorch)
    also uses membrane potentials for its loss computation.
    ...
    def __init__(self, tau=2.0, backend="torch"):
        ...
        self.network = nn.Sequential(
            ...
            # Layer 4 (output): FC only — NO spiking neuron
            # Output is continuous membrane potential, not binary spikes.
            # This matches Method 1 which uses mem_record for loss.
            layer.Linear(800, 10),
        )
    ...
    def forward(self, x):
        ...
        Returns:
            Output membrane potentials of shape (T, batch, 10)
            These are continuous values (NOT binary spikes) since the
            output layer has no spiking neuron.
...
def train_one_epoch(model, train_loader, optimizer, loss_fn, device, T):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # ── Forward pass timing ──
        timer.start()
        out = model(data_seq)  # (T, batch, 10) — membrane potentials, single call

        # Loss: cross-entropy at EACH timestep, summed 
        loss = torch.zeros(1, device=device)
        for step in range(T):
            loss += loss_fn(out[step], target)
...
def evaluate(model, test_loader, device, T):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            out = model(data_seq)  # (T, batch, 10) — membrane potentials

            # Sum membrane potentials across timesteps
